...
use crate::cli::{
    abi_processor, amend_processor, benchmark_processor, build_interactive,
...
use tool::client::basic::Client;
use tool::{Encryption, JsonRpcResponse};
...
pub fn start(url: &str, client: &Client) -> io::Result<()> {
    ...
    let mut cli_dir = dirs::home_dir().unwrap();
    cli_dir.push(".shell");
    if !cli_dir.as_path().exists() {
        fs::create_dir(&cli_dir)?;
    }
    let mut history_file = cli_dir.clone();
    history_file.push("history");
    let history_file = history_file.to_str().unwrap();
    let mut config_file = cli_dir.clone();
...
fn start_rustyline(
    ...
    let colored_prompt = Red.bold().paint(">>> ").to_string();

    let rl_mode = |rl: &mut Editor<BlmCompleter>, config: &GlobalConfig| {
    ...
    let rl_config = Config::builder()
        ...
        .build();
    let helper = BlmCompleter::new(parser.clone());
...
impl<'a, 'b> Completer for BlmCompleter<'a, 'b> {
    type Candidate = Pair;
...
impl<'a, 'b> Highlighter for BlmCompleter<'a, 'b> {
...
impl<'a, 'b> Hinter for BlmCompleter<'a, 'b> {
...
impl<'a, 'b> Helper for BlmCompleter<'a, 'b> {}
