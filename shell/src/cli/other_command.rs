...
use tool::client::basic::{Client, Transfer};
use tool::{JsonRpcParams, ParamsValue, TransactionOptions};
...
pub fn completion_processor(app: &mut App, sub_matches: &ArgMatches) {
    ...
    app.gen_completions_to("shell", shell.parse().unwrap(), &mut io::stdout());
}
#[cfg(test)]
mod test {
