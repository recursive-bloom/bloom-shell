...
use crate::cli::{
    ...
    key_processor, rpc_processor, search_processor, store_processor,
...
const DEFAULT_JSONRPC_URL: &str = "http://127.0.0.1:8545";
...
fn main() {
    ...
    if let Some(m) = args {
        if let Err(err) = match cmd {
            "key" => key_processor(m, &printer, &config),
            "rpc" => rpc_processor(m, &printer, &mut config, client),
            "ethabi" => abi_processor(m, &printer, &config),
            _ => Ok(())
        } {
            printer.eprintln(&Rc::new(err), true);
            process::exit(1);
        }
    } else if let Err(err) = interactive::start(&default_jsonrpc_url, &client) {
